...
export const api = {
  getStocks: (skip: number = 0, limit: number = 50) => request<StocksResponse>(`/companies?skip=${skip}&limit=${limit}`),
  getRegistry: (skip: number = 0, limit: number = 50) => request<StocksResponse>(`/companies/registry?skip=${skip}&limit=${limit}`),
  getCompany: (symbol: string) => request<StockDetailResponse>(`/company/${symbol}`),
  ...
  searchSymbols: (query: string, sector?: string) =>
    request<{ query: string; results: { symbol: string; name: string; sector: string }[]; disclaimer: string }>(
      `/search?q=${encodeURIComponent(query)}${sector ? `&sector=${encodeURIComponent(sector)}` : ""}`,
    ),
  ...
  getPeers: (symbol: string) => request<{ symbol: string; peers: any[] }>(`/peers/${symbol}`),
