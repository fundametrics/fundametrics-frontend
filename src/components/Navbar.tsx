  ...
  return (
    <header className="w-full z-50 transition-all duration-300">
      ...
      <div className="relative w-full px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex-1 max-w-3xl hidden md:flex items-center bg-slate-100/40 backdrop-blur-md border border-slate-200/60 rounded-xl px-1 hover:border-indigo-400/50 hover:bg-white transition-all duration-300 group/search focus-within:ring-2 focus-within:ring-indigo-500/10 focus-within:border-indigo-500">
        ...
        <button
          className="md:hidden p-3 text-slate-500 hover:bg-slate-100 rounded-xl transition-colors active:scale-95"
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden absolute top-[100%] left-0 w-full bg-white/95 backdrop-blur-2xl border-b border-slate-200 p-4 shadow-2xl animate-in slide-in-from-top-2 duration-300 flex flex-col gap-2">
          ...
          <NavLink to="/watchlist" className={linkClass} onClick={() => setMobileMenuOpen(false)}>
            <Star size={18} />
      ...
      {mobileSearchOpen && (
        <div className="md:hidden fixed inset-0 z-50 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="absolute top-0 left-0 right-0 bg-white shadow-2xl animate-in slide-in-from-top-4 duration-300">
